...
class GeminiPageActions:
    ...
    async def ensure_reasoning_mode(self):
        ...
        try:
            ...
            if mode_element:
                ...
                if "Razonamiento" in current_mode or "Thinking" in current_mode:
                    logger.info(f"✓ Already in Reasoning mode ({current_mode})")
                    return

                logger.info(
                    f"Current mode is '{current_mode}', switching to Reasoning/Thinking..."
                )
                ...
                # Try many ways to open the dropdown
                try:
                    await self.page.click(dropdown_icon, force=True, timeout=5000)
                except Exception:
                    # Fallback to general mode selector
                    await self._try_selectors(
                        "mode_selector", action="click", force=True
                    )

                await self.page.wait_for_timeout(2000)

                # Seleccionar "Razonamiento" o "Thinking" del menú
                # We try both Spanish and English fallback if reasoning_option fails or is specific
                reasoning_option = getattr(
                    mode_config, "reasoning_option", "text=Razonamiento"
                )

                # Split options if it's a list (comma separated in JSON)
                options = [opt.strip() for opt in reasoning_option.split(",")]

                found_option = False
                for opt in options:
                    try:
                        # Try to find and click the option
                        locator = self.page.locator(opt)
                        if await locator.count() > 0:
                            await locator.first.click(force=True)
                            found_option = True
                            logger.info(f"Clicked reasoning option: {opt}")
                            break
                    except Exception:
                        continue
                ...
                if not found_option:
                    ...
                    for fallback in [
                        ...
                        try:
                            await self.page.click(fallback, force=True, timeout=2000)
                            found_option = True
                            break
                        except Exception:
                            continue
                ...
                success, details = await self.validator.validate_mode_changed(
                    "Reasoning",
                    timeout=5000,  # Validator should also be updated or be flexible
                )
                if success:
    ...
    async def debug_selectors(self) -> Dict[str, Any]:
        """
        Tests all configured selectors against the current page state.
        Returns a report of which selectors are working and which are not.
        Useful for proactive maintenance when Gemini UI updates.
        """
        ...
        for field in critical_fields:
            selectors = selector_manager.current.get_all_selectors(field)
            field_results = []
            ...
            for selector in selectors:
                try:
                    ...
                    field_results.append(
                        {"selector": selector, "status": "ERROR", "error": str(e)}
                    )

            report["results"][field] = field_results

            # Summary for log
            ok_count = len([r for r in field_results if r["status"] == "OK"])
            if ok_count > 0:
                logger.info(
                    f"✅ Selector '{field}': {ok_count}/{len(selectors)} working"
                )
            else:
                logger.error(f"❌ Selector '{field}': ALL FAILED")

        # Capture a screenshot for visual context during the check
